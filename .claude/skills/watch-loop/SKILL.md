...
2. Barrido /gather-updates COMPLETO: (a) satélites y RSS: node scripts/fetch-firms.mjs && node scripts/fetch-copernicus.mjs && node scripts/fetch-aemet.mjs && node scripts/fetch-news.mjs; interpretar focos <6h, si hay producto Copernicus MON nuevo, y los titulares RSS nuevos (cada uno es pista: abrir y contrastar los relevantes); (b) X: node scripts/fetch-x.mjs (perfiles oficiales; NO se usa el hashtag: `twitter search` da 404 desde el 29-jul, ver lessons/el-hashtag-que-dejo-de-existir). Leer el EXIT CODE: 0 barrido hecho; 2/3/4/5 el barrido NO se hizo → al parte va "X no consultada", jamás "sin novedades", y el 5 (capacidad upstream desaparecida) además se avisa por ntfy. Si el script anuncia que `search` revive, decirlo en el parte. La capa ciudadana (prensa local, vecinos, cortes) NO está cubierta por los perfiles: declararlo y compensar en (c)/(d); (c) live blogs de prensa del registro directory/ vía WebFetch — si un sitio bloquea WebFetch, agent-browser por Bash (open + get text body); (d) WebSearch de cierre "incendio Los Gallardos última hora" con la fecha real; (e) vigilancia expresa Valle del Este / retorno / luz verde 112 / Ayuntamiento de Vera.
