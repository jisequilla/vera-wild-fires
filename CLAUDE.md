...
- `scripts/fetch-copernicus.mjs` / `fetch-firms.mjs` / `fetch-aemet.mjs` — bajan satélites y meteo a `layers.json`; después, proyectar. `fetch-news.mjs` — titulares nuevos vía RSS a `data/news/`; `fetch-x.mjs` — tuits nuevos de los perfiles oficiales a `data/x/`; `fetch-boja.mjs` — publicaciones nuevas del BOJA (ayudas, decretos, el expediente) a `data/boja/`, con el fetch real en `fetch-boja.py` (Scrapling vía `uv run`) como proceso hijo (los tres planos gitignored: efímeros para el parte de gather, no del panel). `notify-changes.mjs` alerta vía ntfy en el deploy (secrets `AEMET_API_KEY`/`NTFY_TOPIC`, opcionales).
...
- El buscador del BOJA es **Solr detrás de un GET plano** (`eboja/buscador/search.do` — la paginación expone `fq`/`sort`/`start`), pero su relevancia no es de fiar: hace stemming (Gallardos≈apellido Gallardo) y **AND de términos multi-palabra** (medido: `incendio forestal Almería`=0 resultados, `incendio`=16). Consultar ancho (ventana de fechas + orden descendente, consultas de una palabra o frase entrecomillada) y filtrar en cliente (`boja.filterPatterns`). Cero-resultados y página-rediseñada son indistinguibles (ni listado ni paginado): lo decide la consulta de control, como la cuenta de control de X.
- Mismo principio en el RSS: **Google News corta a 100 items por consulta sin decirlo**. `fetch-news.mjs` lo detecta y avisa (`⚠` techo rozado · `✗` recorte dentro de la ventana = titulares no vistos). A la escala de un incendio grande el tope se toca en 24 h incluso consultando un solo municipio; estrechar la consulta amplía la cobertura hacia atrás pero no libra del techo.
...
Cuenta personal: `gh auth switch -u jisequilla` antes de cualquier push/PR. Deploy: GitHub Pages (`.github/workflows/deploy.yml`) — **un push a main publica el panel**; por eso todo commit pasa por `/commit`.
